var game = {
    ...
    fold: function (folder) {
        ...
        //log(player);
        this.winner = folder == player.name ? npc.name : player.name;
        log("we have a fold: winner is " + this.winner + " folder was:" + folder);
        this.payout();
    ...
    payout: function () {
        ...
        player.render();
        //log("after fold player stack " + player.stack+ " this.pot  " + this.pot);
        // log(player);
        // log(" this.state " + this.states
    ...
    turn5: function () {
        ...
        if (best1.rank < best2.rank) {
            ...
            log("npc wins:");
            this.payout(npc.name);
            this.result = {winner: cpu.name, rank: best2, is_tie: false};
        } else if (best1.rank == best2.rank) {
            // var hc_cmp = best1.cmp_highcards(best2, 1);
            if (hc_cmp == 1) {
                player.stack = parseInt(player.stack, 10) + parseInt(this.pot, 10);
                log("player wins h:" + player.stack);
                this.result = {winner: player.name, rank: best1, is_tie: false};
            } else if (hc_cmp == -1) {
                npc.stack = parseInt(npc.stack, 10) + parseInt(this.pot, 10);
                log("npc wins h:");
                this.result = {winner: npc.name, rank: best2, is_tie: false};
            } else if (hc_cmp == 0) {
                player.stack = parseInt(player.stack, 10) + parseInt(this.pot / 2, 10);
                npc.stack = parseInt(npc.stack, 10) + parseInt(this.pot / 2, 10);
                log("tie:");
                this.result = {winner: '', rank: best1, is_tie: true};
            }
        } else {
