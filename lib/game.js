var game = {
    ...
    fold: function (folder) {
        ...
        //log(player);
        this.result = {winner: folder == player.name ? npc.name : player.name, rank: {}, is_tie: false};
    ...
    turn5: function () {
        ...
        if (best1.rank < best2.rank) {
            this.result = {winner: player.name, rank: _.clone(best1), is_tie: false};
            log("player wins:" + player.stack);
        } else if (best1.rank > best2.rank) {
            npc.stack = parseInt(npc.stack, 10) + parseInt(this.pot, 10);
            log("npc wins:");
            this.result = {winner: npc.name, rank: _.clone(best2), is_tie: false};
        } else if (best1.rank == best2.rank) {
            // var hc_cmp = best1.cmp_highcards(best2, 1);
            if (hc_cmp == 1) {
                ...
                log("tie:");
                this.result = {winner: '', rank: _.clone(best1), is_tie: true};
            }
        } else {
            log("invalid game state!");
        }
        this.payout(this.result.winner.name);
        menu.show("WINNER");
        log(best1);
        log(best2);
    },
